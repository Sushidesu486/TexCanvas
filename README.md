...
```bash
python -m texcanvas examples/demo.yml \
...
运行 `texcanvas --help` 可查看完整说明：

```text
usage: texcanvas [-h] -o OUTPUT [-t TEMPLATE] [--asset-root ASSET_ROOT]
...
options:
  ...
  -t, --template TEMPLATE
                        可选的可编辑 PPTX 模板路径；保留母版/主题，模板中的示例页会被清空
...
- `-t / --template`：可选的可编辑 PPTX 模板；会保留模板的母版、主题和页面尺寸，模板中的示例页会被清空（输入模板本身不会被修改）。
...
- `equation`：必须提供 `equation` 文本。支持最小化的 LaTeX 风格标记：`\frac{a}{b}`、`\sum`/`\alpha`/`\rightarrow` 等符号命令、`\log`/`\sin` 等函数名、`^{...}` 上标和 `_{...}` 下标。以居中直角面板呈现，不依赖外部 LaTeX。
...
测试覆盖模型不可变性、YAML/schema 校验、section ID、图片资源、contain/cover 几何、导航宽度、页码、CLI、模板不变性、新增的 code/table/equation/block 版式校验与渲染，以及集成生成。集成测试会用 `python-pptx` 重新打开输出，并检查可编辑形状、当前 section 颜色、图片、表格单元格、公式上下标、代码关键字高亮、参考文献、ZIP 格式和关键 OOXML 文件。
...
- `equation` 仅做最小化 LaTeX 文本渲染（分数、上下标、符号命令），不调用 LaTeX 引擎，无法渲染复杂排版公式；TikZ 转 SVG 管线为后续规划。
- 自动测试可以验证 OOXML 与 `python-pptx` 兼容性，但 macOS WPS GUI 打开效果必须由用户进行最终人工验收。

## 项目结构

```text
src/texcanvas/          模型、加载、校验、资源、几何、build/CLI 与渲染器
